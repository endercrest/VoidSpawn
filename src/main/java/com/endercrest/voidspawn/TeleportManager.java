...
public class TeleportManager {
	...
	public boolean teleportSpawn(Player player, String worldName) {
		...
		World world = this.plugin.getServer()
				.getWorld(ConfigManager.getInstance().getString(worldName + ".spawn.world"));
	...
	public boolean teleportTouch(Player p) {
		...
		if (this.playerLocation.get(uuid) == null) {
			...
			loc = this.playerLocation.get(uuid);
		}
		Location below = new Location(loc.getWorld(), loc.getX(), loc.getY() - 1.0D, loc.getZ());
		if (below.getBlock().getType().equals(Material.AIR)) {
			for (int i = 1; i < 10; i++) {
				Location newLoc = new Location(loc.getWorld(), loc.getX() + i,
						loc.getWorld().getHighestBlockYAt(loc.getBlockX() + i, loc.getBlockZ()), loc.getZ());
				Location newLocBelow = new Location(loc.getWorld(), loc.getX() + i,
						loc.getWorld().getHighestBlockYAt(loc.getBlockX() + i, loc.getBlockZ()) - 1, loc.getZ());
			...
			for (int i = 1; i < 10; i++) {
				Location newLoc = new Location(loc.getWorld(), loc.getX(),
						loc.getWorld().getHighestBlockYAt(loc.getBlockX(), loc.getBlockZ() + i), loc.getZ() + i);
				Location newLocBelow = new Location(loc.getWorld(), loc.getX(),
						loc.getWorld().getHighestBlockYAt(loc.getBlockX(), loc.getBlockZ()) - 1, loc.getZ() + i);
				if (!newLocBelow.getBlock().getType().equals(Material.AIR)) {
					p.setFallDistance(0.0F);
					p.teleport(newLoc);
					return true;
	...
	public boolean teleportIsland(Player p) {
		if (VoidSpawn.IslandWorld) {
			...
			if ((ASkyBlockAPI.getInstance().hasIsland(p.getUniqueId()))
					|| (ASkyBlockAPI.getInstance().inTeam(p.getUniqueId()))) {
